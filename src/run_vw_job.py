...
def process(ds, params, results_dir, add_parse = None, data_choice = 'loan'):
    print('processing', ds, params, data_choice)
    ...
    for k, v in params.items():
        if k == 'alg':
            ...
            #bandit learning
                ...
                if data_choice == 'ms' or data_choice == 'yahoo' or data_choice == 'loan':
                    cmd += ['--cbify_ldf']
                else:
...
if __name__ == '__main__':
    ...
    if not args.test:
        print("yes")
        fname = os.path.join(args.results_dir, 'all_lossesx.txt')
        done_tasks = set()
        print(fname)
    ...
    while idx < tot_jobs:
        ...
        if args.test:
            ...
            if task_name not in done_tasks and not skip_params(params):
                try:
                    pv_loss, all_output = process(ds, params, args.results_dir, None, data_choice)
                    loss.append(pv_loss)
